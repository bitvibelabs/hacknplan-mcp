...
SCHED_BUCKETS = [
    ("Overdue", "#DE350B", lambda d: d < 0),
    ("This week", "#FF8B00", lambda d: 0 <= d <= 7),
    ("Next 2 weeks", "#FFC400", lambda d: 8 <= d <= 14),
    ("This month", "#4C9AFF", lambda d: 15 <= d <= 30),
    ("Later", "#7A869A", lambda d: d > 30),
]


def schedule_section(data: dict) -> str:
    sched = data.get("schedule", [])
    if not sched:
        return ""
    cols = []
    for label, color, pred in SCHED_BUCKETS:
        ...
        for s in rows:
            d = s["days_left"]
            cd = f"{-d}d over" if d < 0 else ("today" if d == 0 else f"{d}d")
            items.append(
                f'<li><span class="cd" style="color:{color}">{cd}</span>'
                f'<span class="dl-title">{esc(s["title"][:54])}</span>'
                f'<span class="dl-proj">{esc(s["project"])} · {s["due"]}</span></li>')
        cols.append(f'''<div class="sched-col">
          <h3 style="border-color:{color}">{esc(label)} <span>{len(rows)}</span></h3>
          <ul>{"".join(items)}</ul></div>''')
    return f'''<section>
      <h2 style="border-color:#DE350B"><span class="dot" style="background:#DE350B"></span>
        Schedule — deadline countdown <span class="wsum">{len(sched)} dated items</span></h2>
      <div class="sched">{"".join(cols)}</div>
    </section>'''
...
def render(data: dict) -> str:
    ...
    sched_html = schedule_section(data)
    sections = []
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  .sched {{ display:grid; grid-template-columns:repeat(auto-fit,minmax(220px,1fr)); gap:14px; }}
  .sched-col h3 {{ font-size:12px; margin:0 0 8px; border-left:3px solid; padding-left:8px;
                   text-transform:uppercase; letter-spacing:.04em; color:#c9d1d9; }}
  .sched-col h3 span {{ color:#8b949e; }}
  .sched-col ul {{ list-style:none; margin:0; padding:0; display:flex; flex-direction:column; gap:6px; }}
  .sched-col li {{ background:#161b22; border:1px solid #30363d; border-radius:8px; padding:7px 10px;
                   display:grid; grid-template-columns:auto 1fr; gap:2px 8px; align-items:baseline; }}
  .cd {{ font-weight:700; font-size:12px; white-space:nowrap; grid-row:span 2; }}
  .dl-title {{ font-size:12px; color:#e6edf3; }}
  .dl-proj {{ font-size:10px; color:#8b949e; }}
